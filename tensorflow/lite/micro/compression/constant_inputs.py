...
_REQUIREMENTS: dict[tuple[int, int], Requirement] = {
  (tflite.BuiltinOperator.PAD, 1): Requirement(
    "paddings", "kernels/pad_common.cc"
  ),
  (tflite.BuiltinOperator.PADV2, 1): Requirement(
    "paddings", "kernels/pad_common.cc"
  ),
  (tflite.BuiltinOperator.TRANSPOSE, 1): Requirement(
    "perm", "kernels/transpose_common.cc"
  ),
  (tflite.BuiltinOperator.STRIDED_SLICE, 1): Requirement(
    "begin", "kernels/strided_slice_common.cc"
  ),
  (tflite.BuiltinOperator.STRIDED_SLICE, 2): Requirement(
    "end", "kernels/strided_slice_common.cc"
  ),
  (tflite.BuiltinOperator.STRIDED_SLICE, 3): Requirement(
    "strides", "kernels/strided_slice_common.cc"
  ),
  (tflite.BuiltinOperator.EXPAND_DIMS, 1): Requirement(
    "axis", "kernels/expand_dims.cc"
  ),
  (tflite.BuiltinOperator.FILL, 0): Requirement("dims", "kernels/fill.cc"),
  (tflite.BuiltinOperator.BROADCAST_TO, 1): Requirement(
    "shape", "kernels/broadcast_to.cc"
  ),
  (tflite.BuiltinOperator.SPLIT, 0): Requirement("axis", "kernels/split.cc"),
  (tflite.BuiltinOperator.SPLIT_V, 2): Requirement(
    "axis", "kernels/split_v.cc"
  ),
  (tflite.BuiltinOperator.RESIZE_BILINEAR, 1): Requirement(
    "size", "kernels/resize_bilinear.cc"
  ),
  (tflite.BuiltinOperator.RESIZE_NEAREST_NEIGHBOR, 1): Requirement(
    "size", "kernels/resize_nearest_neighbor.cc"
  ),
}
