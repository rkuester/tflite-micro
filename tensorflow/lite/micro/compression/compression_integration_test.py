...
class LutCompressionTest(tf.test.TestCase):
  ...
  def test_lut_decode_operators_present(self):
    ...
    decode_ops = [
        ...
        if op.opcode == tflite.BuiltinOperator.CUSTOM and
        op.custom_code == decode_insert.DECODE_CUSTOM_OP_NAME
    ]

    self.assertGreater(len(decode_ops), 0, "DECODE operators should be present")
