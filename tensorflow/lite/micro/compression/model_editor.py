...
class Quantization:
  ...
  def to_tflite(self) -> tflite.QuantizationParametersT:
    ...
    # Normalize to lists
    scales = [self.scales] if isinstance(self.scales, (int, float)) else self.scales
    zeros = [self.zero_points] if isinstance(self.zero_points, int) else self.zero_points
